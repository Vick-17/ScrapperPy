...
def main():
    ...
    print('Collection and saving data')
...
def get_years(soup):
    # Crée une compréhension de liste en appliquant get_text() fonction sur tous les éléments du résultat de la méthode find_all().
    years = [year.get_text() for year in soup.find_all(
        # crée une regex pour rechercher les balises ayant 'sub-menu' comme sous-chaîne dans son nom de classe.
        'a', class_=re.compile(r'sub-menu'))]
    years[-1] = 'all-time'
    return years


def get_stats(soup):
    # Recherche toutes les balises 'a' avec le nom de classe spécifié.
    stats = soup.find_all('a', class_=re.compile(r'side*'))
    stats_url = [re.search(r'\d/(.*)', stat['href']).group(1)
                 for stat in stats]
...
def prepare_question(years, stats_title, stats_url):
    ...
    stats_dic_list = [{'name': stats_title[i], 'value': stats_url[i]}
                      for i in range(len(stats_title))]
...
def find_col(soup, team):
    try:
        if team:
            columns = list(filter(None, soup.find(
                'tr', class_='standings-table_header').get_text().split('\n')))
...
def get_team_data(soup, columns):
    ...
    for i in soup.find_all('td'):
        ...
        data = np.array(data).reshape(
            len(data) // len(columns), len(columns) + 1)
...
def get_player_data(soup, columns):
    data = []
    for i in soup.find_all('td', class_=re.compiler(r'top-players*')):
        data.append(re.sub(r'\n[\s]*', " ", i.get_text().strip()))
...
def save_data(data, columns, data_set_name, file_name):
    ...
    print(data_set_name + 'save as: ' +
          os.path.dirname(os.path.abspath(__file__)) + '/' + data_set_name + '.csv')
